...
const UpdateProfile = ({
  ...
  const { firstname, lastname, email, location, dataNastere } = input;
      ...
      {!loading ? (
        <>
          <div className={[stylesHere.profileDesc, "universalMt"].join(" ")}>
            ...
            <p>
              Register Date:{" "}
              <Moment className="bold" format="YYYY-MM-DD">
                {user.date}
              </Moment>
            </p>
          </div>
          <form
            className={["universalForm", "universalFormMt"].join(" ")}
            onSubmit={(e) => updateUserProfileNow(e)}
          >
            {alerts.length > 0 && (
              <div className="alerts">
                {alerts.map((alert) => (
                  <span key={alert.id} className={`alert alert-${alert.type}`}>
                    {alert.msg}
                  </span>
                ))}
              </div>
            ...
            <div className={stylesLoginAdmin.inputGroup}>
              ...
              <div
                className={[
                  stylesLoginAdmin.inputRegister,
                  stylesLoginAdmin.doctorIcon,
                ].join(" ")}
              >
                <input
                  type="text"
            ...
            <input
              type="submit"
              className={["universalBtn", styles.addBtn].join(" ")}
              value="update "
            />
          </form>
        </>
